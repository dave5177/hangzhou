...
//		g.drawString("单位长度:" + length_unit, 203, 73, 0);
	...
	public void loadResource() {
		try {
//			img_map = Image.createImage("/game/map_" + HSProp.getRandom(0, 2) + ".png");
			img_map = Image.createImage("/game/map_1.png");
...
//				num_coin -= 100;
	...
	public void createBird() {
		...
		for (int i = 0; i < 5; i++) {
			int type_temp = HSProp.getRandom(0, type_ctrl - 1);
			...
			if(treasure_info[i][2] <= 0) {
				if(treasure_info[i][0] > 5) {
					...
					if(treasure_info[i][1] > 100){
						treasure_info[i][1] = 0;
						v_treasure.addElement(new Treasure(this, HSProp.getRandom(170, 470), 0, i));
					}
