...
public class Dialog implements Showable {
	...
	private Image img_own_yuanbao;//你当前的元宝数
	...
	private void showGoodsPrice(Graphics g) {
		C.drawString(
				g,
				img_number,
				canvasControl.anHui_Tool
						.getPrice(CanvasControl.A_GOODS_PARAM[CanvasControl.goodsIndex][0]),
				"0123456789.", 235, 208, 10, 15, 0, 0, 0);
		g.drawImage(img_unit_price,
				235 + canvasControl.anHui_Tool
				.getPrice(CanvasControl.A_GOODS_PARAM[CanvasControl.goodsIndex][0])
						.length() * 10, 206, 0);
		
		g.drawImage(img_own_yuanbao, 230, 365, 0);
		C.drawString(g, img_number, CanvasControl.userCoin, "0123456789.", C.WIDTH / 2 + 80, 368, 10, 15, 0, 0, 0);
	}
	...
	public void loadResource() {
		try {
			...
			if (type == 5) {
				...
				img_own_yuanbao = Image.createImage("/dialog/own_yuanbao.png");
				...
				for (int i = 0; i < a_img_info_zj.length; i++) {
					a_img_info_zj[i] = Image.createImage("/dialog/info_" + i
							+ ".png");
				}
			} else {
	...
	public void keyPressed(int keyCode) {
		...
		case C.KEY_FIRE:
			if (type == 2) {// 选好英雄开始游戏，没有则弹出购买对话框。
				if (index > 0) {
					if (CanvasControl.goodsNumber[index - 1] > 0) {
						...
						this.removeResource();
						canvasControl
								.setView(new Dialog(canvasControl, 5, this));
					}
				} else {
					CanvasControl.type_hero = index;
					canvasControl.setView(canvasControl.nullView);
			...
			if (index == 0) {
				...
				case 5:// 购买道具
					canvasControl.anHui_Tool
							.buyProp(CanvasControl.A_GOODS_PARAM[CanvasControl.goodsIndex][0]);
					break;
				case 6:
					lastView.removeServerImage();
					canvasControl.setGoBackView(null);
