...
//		img_bunker = null;
	...
	public void keyPressed(int keyCode) {
		if(CanvasControl.firstTimes == 1) {//教学篇按键事件
			...
			case C.KEY_DOWN :
				if(showGameOverDialog) {
					if(gameOverSelectIndex < 2) gameOverSelectIndex ++;
					return;
				}
				keyLeftIsDown = false;
				keyDownIsDown = true;
			...
			case C.KEY_UP :
				if(showGameOverDialog) {
					if(gameOverSelectIndex > 0) gameOverSelectIndex --;
					return;
				}
				keyLeftIsDown = false;
				keyDownIsDown = false;
...
//						gameOverX = 660;
	...
	public void doKeyIsDown() {
		if(keyLeftIsDown) {
			if(sightX > 0) {
				if(sightMoveSpeed < 60) sightMoveSpeed += 3;
...
//				gunY -= sightMoveSpeed / 3;
		...
		if(!keyLeftIsDown && !keyDownIsDown && !keyRightIsDown && !keyUpIsDown) sightMoveSpeed = 3;
	}
...
//				gunY = sightY / 3 + 318;
	 ...
	 * 射击。
	 */
	public void shoot(AudioPlaydalin player) {
		gun.shoot(player);
		shootTimes ++;
