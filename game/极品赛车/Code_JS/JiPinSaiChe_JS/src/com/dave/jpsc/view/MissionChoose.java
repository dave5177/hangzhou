...
public class MissionChoose implements Showable {
	...
	private void showTitle(Graphics g) {
		...
		g.drawString("奖励", x_param + 8, y_param + 290, 0);
	...
	private void showMissionInfo(Graphics g) {
		...
		C.drawStrInRect(x_param + 8, y_param + 55, 300, 70,
				missionInfo[index][0], C.FONT_SMALL_PLAIN, g);
		C.drawStrInRect(x_param + 8, y_param + 180, 300, 70, "圈数："
				+ missionInfo[index][1], C.FONT_SMALL_PLAIN, g);
		C.drawStrInRect(x_param + 140, y_param + 180, 300, 70, "对手数量："
				+ missionInfo[index][2], C.FONT_SMALL_PLAIN, g);
		C.drawStrInRect(x_param + 8, y_param + 204, 300, 70, "比赛类型："
				+ missionInfo[index][3], C.FONT_SMALL_PLAIN, g);
		C.drawStrInRect(x_param + 8, y_param + 228, 300, 70, "完成条件："
				+ missionInfo[index][4], C.FONT_SMALL_PLAIN, g);
		C.drawStrInRect(x_param + 8, y_param + 270, 300, 70, "难度：",
				C.FONT_SMALL_PLAIN, g);
		for (int i = 0; i < 5; i++) {
			g.drawImage(a_img_star[1], x_param + 80 + i * 19, y_param + 278,
					Graphics.VCENTER | Graphics.HCENTER);
		}
		int count = Integer.parseInt(missionInfo[index][5]);
		for (int i = 0; i < count; i++) {
			g.drawImage(a_img_star[0], x_param + 80 + i * 19, y_param + 278,
					Graphics.VCENTER | Graphics.HCENTER);
		}

		C.drawStrInRect(x_param + 8, y_param + 310, 300, 70, "经验："
				+ missionInfo[index][6], C.FONT_SMALL_PLAIN, g);
	}
