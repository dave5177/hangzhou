...
public class Car implements Model {
	...
	private Image[] a_img_reject;//排斥效果
...
//					if (lap < CanvasControl.MISSIONPROPERTY[CanvasControl.mission - 1][0]) {
	 ...
	 * 排斥道具效果
	 */
	public void loadRejectImage() {
		try {
			Image reject = Image.createImage("/game/reject.png");
			
			a_img_reject = new Image[3];
			for (int i = 0; i < 3; i++) {
				a_img_reject[i] = Image.createImage(reject, i * 87, 0, 87, 86, 0);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	 ...
	 * @param g
	 */
	public void showReject(Graphics g) {
		g.drawImage(a_img_reject[frame_rej], x + game.x_map, y + game.y_map, Graphics.VCENTER | Graphics.HCENTER);
	}
