...
public class Loading implements Showable, Runnable {
	 ...
	 * 昵称异常，需要更改
	 */
	public static boolean nickNameExpt;
...
//		g.setClip(0, 0, 640, 530);
		...
		if (type == 100) {
			...
			if(nickNameExpt) {
				g.drawString(canvasControl.me.nickName, 200, 185, 0);
				g.setFont(C.FONT_SMALL_PLAIN);
				g.setColor(0xff0000);
				g.drawString("您的昵称异常，请更换昵称", 320, 265, Graphics.TOP | Graphics.HCENTER);
			} else {
		...
		g.setColor(255, 255, 255);
		g.setFont(C.FONT_SMALL_PLAIN);
		g.drawString(CanvasControl.VERSION + AH_TOOL.version, 630, 10, Graphics.RIGHT | Graphics.TOP);
		...
		if (type == 100) {
			...
			case C.KEY_FIRE:
				if (index_choose == 0) {// 更换
					...
					if(!nickNameExpt && !CanvasControl.nickFromSer.equals("昵称获取中，请稍后") && !CanvasControl.nickFromSer.equals("")) {
						if(canvasControl.me == null) {
							int[][] myCars = {
									{0, 100, 100, 100, 0}
							};
							canvasControl.me = new Player(CanvasControl.nickFromSer, 1, 100, 0, 10, 0, myCars, CanvasControl.iptvID, canvasControl);
						} else {
							canvasControl.me.nickName = CanvasControl.nickFromSer;
						}
	...
	public void run() {
		switch (type) {
		case 0: // 首页
//			home.a_img_back = getImage("/home/back.jpg");
