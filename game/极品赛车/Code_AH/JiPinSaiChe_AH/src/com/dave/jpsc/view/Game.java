...
public class Game implements Showable {
	 ...
	 * 地图的高和宽
	 */
	public int width_map, height_map;

	public Image[] a_img_map;
	 ...
	 * 地图分割后一张图片的宽和高
	 */
	private int width_pieceMap, height_pieceMap;
	...
	public void init() {
		width_pieceMap = a_img_map[0].getWidth();
		height_pieceMap = a_img_map[0].getHeight();

		width_map = width_pieceMap * 2;
		height_map = height_pieceMap * 2;
		...
		if (myCar.getType() < recomIndex
				|| canvasControl.me.getMyCarIndex(recomIndex) == -1) {
			Dialog.game = this;
	...
	public void show(Graphics g) {
		g.drawImage(a_img_map[0], x_map, y_map, 0);
		g.drawImage(a_img_map[1], x_map + width_pieceMap, y_map, 0);
		g.drawImage(a_img_map[2], x_map, y_map + height_pieceMap, 0);
		g.drawImage(a_img_map[3], x_map + width_pieceMap, y_map
				+ height_pieceMap, 0);
		...
		if (myCar.blindTime > 0) {
			...
			g.drawString("你可以修复你的车辆减少被对手击中的概率", 320, 240, Graphics.HCENTER
					| Graphics.BOTTOM);
			g.setColor(0xb12812);
			g.drawString("按1键解除致盲", 320, 280, Graphics.HCENTER
					| Graphics.BOTTOM);
		}
	...
	public void rollMap() {
		...
		if (y_map > 0)
			y_map = 0;
		else if (y_map < 530 - height_map)
			y_map = 530 - height_map;
	}
