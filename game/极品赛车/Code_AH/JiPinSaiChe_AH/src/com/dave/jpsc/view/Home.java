...
public class Home implements Showable {
	...
	public void show(Graphics g) {
		for (int i = 0; i < 4; i++) {
			g.drawImage(a_img_back[i], i * 160, 0, 0);
		}
		...
		if (PlayerHandler.soundClosed) {
			g.drawImage(a_img_word_sound[0], x_word_sound, 528, Graphics.BOTTOM
					| Graphics.HCENTER);
			g.drawImage(a_img_word_sound[1], x_word_sound + 4, 528,
					Graphics.BOTTOM | Graphics.HCENTER);
		} else {
			g.drawImage(a_img_word_sound[0], x_word_sound, 528, Graphics.BOTTOM
					| Graphics.HCENTER);
			g.drawImage(a_img_word_sound[2], x_word_sound + 4, 528,
					Graphics.BOTTOM | Graphics.HCENTER);
		}

		g.setColor(255, 255, 255);
		g.drawString(CanvasControl.VERSION + AH_TOOL.version, 10, 10, 0);
	}
	...
	private void showMsg(Graphics g) {
		g.drawRegion(img_msg, msg_frame * 54, 0, 54, 52, 0, 550, 200,
				Graphics.HCENTER | Graphics.BOTTOM);
		g.drawImage(img_number_back, 580, 165, Graphics.HCENTER
				| Graphics.VCENTER);
		C.drawString(g, img_number, Message.numbers + "", "0123456789", 580,
				173, 10, 15, Graphics.HCENTER | Graphics.BOTTOM, 0, 0);

		g.setClip(500, 180, 100, 50);

		g.drawImage(img_msg_word, x_msg_word, 185, Graphics.TOP
				| Graphics.HCENTER);
		g.setClip(0, 0, 640, 530);
	}
	...
	public void loadResource() {
		try {
			...
			for (int i = 0; i < 4; i++) {
				a_img_back[i] = Image.createImage("/home/back_" + i + ".jpg");
			}
			...
			for (int i = 0; i < a_img_word_sound.length; i++) {
				a_img_word_sound[i] = Image.createImage("/home/word_sound_" + i
						+ ".png");
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void logic() {
		...
		if (!CanvasControl.handledDay) {
			if (CanvasControl.playDay != 0 && CanvasControl.lastPlayDay != 0) {
				...
				if (CanvasControl.playDay != CanvasControl.lastPlayDay
