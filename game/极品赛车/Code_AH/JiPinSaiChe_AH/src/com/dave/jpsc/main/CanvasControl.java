...
import AH_PROP_TOOL.AH_Listenner;
import AH_PROP_TOOL.AH_TOOL;
 ...
 * 
 */
public class CanvasControl extends Canvas implements Runnable, AH_Listenner {
	...
	public final static String VERSION = "Version_AH_2.0.0";

	/**
	 * 浙江工具
	 */
	public AH_TOOL ah_tool;
	 ...
	 * 游戏图片存放的服务器地址
	 */
	public static String imageServerUrl = "昵称获取中，请稍后";// 从jad获取
	...
	public CanvasControl(MainMIDlet midlet) {
		...
		ah_tool = new AH_TOOL(midlet, this, GAME_PROP_CODE);
//		 ah_tool.setTest();
//		 ah_tool.setTestIptvid("90000231");
	...
	protected void paint(Graphics g) {
		view.show(g);
		// g.setColor(0xffffff);
		// g.drawString(imageServerUrl, 5, 5, 0);
	}
	...
	private void getServerInfo(MIDlet midlet) {
		...
		imageServerUrl = midlet.getAppProperty("ImageServerUrl_AH");
		if (imageServerUrl == null) {
			// imageServerUrl = "http://222.46.20.151:8055/jpsc";// 本地地址
			imageServerUrl = "http://61.191.44.224:8080/GameResource/jpsc";// 安徽正式地址
		}
	}
	...
	public void handleParam(String param) {
		...
		if (a_s_param[1].length() > 2) {
			CanvasControl.missionPassed = 1;
			int[][] myCars = { { 0, 100, 100, 100, 0 } };
			me = new Player("", 1, 100, 0, 10, 0, myCars, CanvasControl.iptvID,
					this);
			return;
		}
	...
	// 严菊芳安徽订购包订购处理
	public void AH_Result(int result, int type) {
		switch (result) {
		case AH_TOOL.BUY_PROP_SUCCESS:
			if (!Game.timeOut && !Game.fuelOut)
				setView(goBackView);
			doBuySuccess();
			break;
		case AH_TOOL.BUY_PROP_FAIL:
		case AH_TOOL.BUY_PROP_ESC:
