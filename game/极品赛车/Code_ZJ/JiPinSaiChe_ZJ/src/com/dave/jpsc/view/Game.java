...
public class Game implements Showable {
	 ...
	 * 声音播放异常
	 */
	public static boolean musicErr;
	...
	public void init() {
		...
		int recomIndex = CanvasControl.mission - 3;
		if(recomIndex < 0)
			recomIndex = 0;
		else if(recomIndex > 8)
			recomIndex = 8;
		
		if(myCar.getType() < recomIndex || canvasControl.me.getMyCarIndex(recomIndex) == -1) {
			Dialog.game = this;
			Dialog.carIndex = recomIndex;
			canvasControl.setView(canvasControl.nullView);
			this.removeResource();
			canvasControl.setView(new Dialog(canvasControl, 11, this));
		}
	}
	...
	private void playerInit() {
		...
		case 15:
			...
			break;
		case 16:
			myCar = new Car(this, 434, 630, 180,
					canvasControl.me.cars[CanvasControl.usingCar][0],
					canvasControl.me, 1);

			enemyCars.removeAllElements();
			enemyCars.addElement(new EnemyCar(this, 434, 600, 180, 4, 1));
	...
	private void initMap_2() {
		...
		circles.addElement(new Circle(717, 405, 97, Circle.OUT_DETECT, 90, 180,
				115));
		circles.addElement(new Circle(837, 398, 117, Circle.OUT_DETECT, 315, 360, 180));
		circles.addElement(new Circle(837, 398, 117, Circle.OUT_DETECT, 0, 90, 180));
	...
	private void initMap_4() {
		...
		freeRects.addElement(new RectFree(206, 66, new Vector2(24, 461), 88));
		freeRects.addElement(new RectFree(155, 48, new Vector2(45, 523), 78));
		freeRects.addElement(new RectFree(153, 54, new Vector2(74, 586), 66));
		freeRects.addElement(new RectFree(128, 46, new Vector2(110, 634), 52));
	...
	private void initMap_8() {
		...
		freeRects.addElement(new RectFree(179, 50, new Vector2(953, 388), 79));
	...
	public void show(Graphics g) {
		...
		for (int i = treasures.size() - 1; i >= 0; i--) {
			((Treasure) treasures.elementAt(i)).show(g);
		}
		...
		for (int i = bullets.size() - 1; i >= 0; i--) {
			((Bullet) bullets.elementAt(i)).show(g);
		}
		...
		if (myCar.slowTime > 0) {
			if (y_word == 200)
				...
				y_word = 200;
			g.setColor(0xffffff);
			g.fillRect(350, y_word - 25, 300, 30);
			g.setColor(0xc72216);
			g.setFont(C.FONT_LARGE_BLOD);
			g.drawString("你被减速了，按1键解除。", 500, y_word, Graphics.HCENTER
					| Graphics.BOTTOM);
		}
		...
		if (myCar.blindTime > 0) {
			...
			g.drawString("你可以修复你的车辆减少被对手击中的概率", 320, 240, Graphics.HCENTER
					| Graphics.BOTTOM);
			g.setColor(0xb12812);
			g.drawString("按1键解除致盲效果", 320, 280, Graphics.HCENTER | Graphics.BOTTOM);
		}
		...
		g.drawImage(a_img_pointer[pointerIndex], 482, 490, Graphics.VCENTER | Graphics.HCENTER);// 速度表盘
...
//		}
	...
	public void loadResource() {
		if(!PlayerHandler.soundClosed) {
			sound_Car = new Sound("/sound/car.wav", false);
			sound_Car.setVolume(100);
			sound_Car.setCount(-1);
		}
		
		if (state == STATE_REDYGO)
			loadReadyImage();
		else if(!PlayerHandler.soundClosed)
			sound_Car.play();
			
		if(myCar.dirErro)
			loadDirErroImage();
		try {
			img_dodge = Image.createImage("/game/dodge.png");
			img_bottom = Image.createImage("/game/bottom.png");
			a_img_pointer = new Image[23];
			for (int i = 0; i < 23; i++) {
				a_img_pointer[i] = Image.createImage("/pointer/pointer_" + i + ".png");
			}
	...
	public void removeResource() {
		...
		img_point = null;
		img_dodge = null;
		if(a_img_pointer != null) {
			for (int i = 0; i < 23; i++) {
				a_img_pointer[i] = null;
			}
			a_img_pointer = null;
		}
	...
	public void keyPressed(int keyCode) {
		if (state == STATE_GAMING) {
			...
			case C.KEY_RIGHT:
				break;
			case C.KEY_1:// 排斥道具
				canvasControl.buyGoods(9, false);
				break;
			case C.KEY_2:// 补充时间
				canvasControl.buyGoods(10, false);
				break;
			case C.KEY_3:// 急速模式
				canvasControl.buyGoods(11, false);
				break;
			case C.KEY_4:// 补充燃油
				canvasControl.buyGoods(12, false);
				break;
			case C.KEY_5:// 减速攻击
				canvasControl.buyGoods(13, false);
				break;
			case C.KEY_6:// 致盲攻击
				canvasControl.buyGoods(14, false);
				break;
			case C.KEY_9:
...
//			canvasControl.playerHandler.playByIndex(0);
		...
		for (int i = treasures.size() - 1; i >= 0; i--) {
			((Treasure) treasures.elementAt(i)).logic();
		}
		
		for (int i = bullets.size() - 1; i >= 0; i--) {
			((Bullet) bullets.elementAt(i)).logic();
		}
	...
	private void addTreasure() {
		if(addTreasureTime <= 0) {
			if(treasures.size() < 5) {
				int dirRectIndex = C.R.nextInt(dirRects.size());
				RectFree dirRect = (RectFree) dirRects.elementAt(dirRectIndex);
				treasures.addElement(new Treasure(this, (int)dirRect.center.x + C.R.nextInt(60) - 30, (int)dirRect.center.y + C.R.nextInt(60) - 30, C.R.nextInt(6) + 9));
			}
			
			addTreasureTime = C.R.nextInt(100) + 100;
		}
		addTreasureTime --;
	}
	...
	public void removeServerImage() {
		...
		if (a_img_cover != null && num_cover > 0) {
	...
	public void blindAttack() {
		for (int i = enemyCars.size() - 1; i >= 0; i--) {
			EnemyCar ec = (EnemyCar) enemyCars.elementAt(i);
			bullets.addElement(new Bullet(this, myCar.x, myCar.y, 1, ec));
//			((EnemyCar) enemyCars.elementAt(i)).beBlindHitted();
	...
	public void slowAttack() {
		for (int i = enemyCars.size() - 1; i >= 0; i--) {
			EnemyCar ec = (EnemyCar) enemyCars.elementAt(i);
			bullets.addElement(new Bullet(this, myCar.x, myCar.y, 0, ec));
