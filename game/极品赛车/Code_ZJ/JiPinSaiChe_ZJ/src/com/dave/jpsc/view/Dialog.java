...
public class Dialog implements Showable {
	...
	/**
	 * 对话框类型控制 0：退出整个程序对话框。 1：退出游戏对话框。2：该车已是顶级。 3：购买成功。 4：购买失败。 5：购买道具。 6：闯关成功。
	 * 7：燃油耗尽。8：时间耗尽。 10：当月购买已达到峰值。11：赛车推荐。
	 */
	private int type;
		...
		case C.KEY_FIRE:
			if (index == 0) {
				...
				case 6:
					...
					break;
				case 7:// 购买燃油
					canvasControl.buyGoods(12, true);
					break;
				case 8:// 时间耗尽，购买时间
					canvasControl.buyGoods(10, true);
					break;
				case 11://使用推荐车辆
					...
					if (mycarIndex >= 0) {//有这辆车
						...
						canvasControl.buyGoods(carIndex, false);
					}
					break;
				...
				if (type == 7 || type == 8) {// 时间或者燃油耗尽
					 ...
					 * CanvasControl.usingCar = canvasControl.me.mainCar;
					 * 
					 * if (game.getMode() != Game.MODE_MISSION) {// 关卡模式
					 * game.duelPlayer.strength += 100;
					 * 
					 * game.duelPlayer.upLoadMsg(Player.WIN,
					 * CanvasControl.iptvID); game.duelPlayer.updateParam();
					 * game.duelPlayer.saveScore();
					 * 
					 * lastView.removeServerImage();
					 * canvasControl.setGoBackView(null);
					 * canvasControl.setView(new Loading(canvasControl, 2)); }
					 * else { lastView.removeServerImage();
					 * canvasControl.setGoBackView(null);
					 * canvasControl.setView(new MissionChoose(canvasControl));
					 * }
					 * 
					 * canvasControl.me.cars[CanvasControl.usingCar][1] -= 6;
					 * canvasControl.me.cars[CanvasControl.usingCar][2] -= 4;
					 * canvasControl.me.cars[CanvasControl.usingCar][3] -= 5;
					 * 
					 * for (int i = 1; i < 4; i++) { if
					 * (canvasControl.me.cars[CanvasControl.usingCar][i] < 0) {
					 * canvasControl.me.cars[CanvasControl.usingCar][i] = 0; } }
					 * CanvasControl.gamesTotal++; canvasControl.saveScore();
					 * canvasControl.saveParam(); game = null;
					 */
		...
		case C.KEY_BACK_ZX:
			...
			if (type == 7 || type == 8) {
				...
				if (game.getMode() != Game.MODE_MISSION) {// 挑战模式
					...
					game.duelPlayer.upLoadMsg(Player.WIN, CanvasControl.iptvID);
					game.duelPlayer.updateParam();
					game.duelPlayer.saveScore();
