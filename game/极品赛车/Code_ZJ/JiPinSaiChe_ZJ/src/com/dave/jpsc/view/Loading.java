...
public class Loading implements Showable, Runnable {
	 ...
	 * 我的车库。7:游戏区。8：关卡结算界面。9：副本选择。100：输入昵称界面。101：空load界面（不会主动跳转）。
	 */
	...
	public static boolean connErr = false;//网络连接错误
...
//		g.setColor(255, 255, 255);
//		g.drawString(CanvasControl.VERSION, 630, 10, Graphics.RIGHT | Graphics.TOP);
		
//		if (connErr) {
//			g.setColor(0xffffff);
//			g.fillRect(200, 200, 440, 50);
//			g.setColor(0);
//			g.drawString("网络连接错误", 320, 205, Graphics.HCENTER | Graphics.TOP);
//		}
		...
		if (type == 100) {
			...
			case C.KEY_FIRE:
				if (index_choose == 0) {// 更换
					...
					if(!CanvasControl.nickFromSer.equals("傻逼们") && !CanvasControl.nickFromSer.equals("") && CanvasControl.nickFromSer != null) {
						int[][] myCars = {
								{0, 100, 100, 100, 0}
						};
...
//			}
		...
		case 6:// 我的车库
			...
			break;
		case 7:// 游戏
			game.img_map = getImage("/map/map_" + CanvasControl.mission
					+ ".jpg");
			if(game.num_cover > 0) {
