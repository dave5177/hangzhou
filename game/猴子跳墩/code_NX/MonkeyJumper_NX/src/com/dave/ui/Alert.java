...
//				img_yuanBao = Image.createImage("/alert/yuanBao.png");
	...
	public void removeSourse() {
		img_dialog = null;
		img_out_word = null;
		img_select = null;
		img_buy_word = null;
		img_buy_success_word = null;
		img_relive_word = null;
		img_js_sm = null;
	...
	public void showMe(Graphics canvas) {
		...
		case C.A_TYPE_BUY : {
			canvas.drawImage(img_select, C.WIDTH / 2 - 60 + index * 160, C.HEIGTH / 4 + 160,
					Graphics.TOP | Graphics.RIGHT);
			String price = canvasControl.nx_iptv_prop_tool.getSpecificPropsPrice(CanvasControl.GAME_PROP_TimeCode_life);
...
//			canvasControl.audioPlay.playSound((byte)2);
			...
			case C.A_TYPE_OUT : {
				if(index == 0) {
					...
					CanvasControl.totalScore += CanvasControl.game_score;
					CanvasControl.game_score = 0;
...
//					canvasControl.nx_iptv_prop_tool.do_BuyProp(img_js_sm, CanvasControl.GAME_PROP_TimeCode_life);
				}
				else if(index == 1) {
					canvasControl.state = CanvasControl.NULL;
