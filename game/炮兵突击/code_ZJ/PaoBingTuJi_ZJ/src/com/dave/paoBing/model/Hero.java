...
//		showPoint(g);
		g.drawImage(game.a_img_hero[angle_index][frame], x + game.x_map, y, Graphics.BOTTOM
				| Graphics.HCENTER);
		showBloodBar(g);
	...
	public void showPoint(Graphics g) {
		if(angle_index < 5) {
			g.drawImage(game.a_img_point[angle_index], x_muzzle, y_muzzle, Graphics.BOTTOM | Graphics.LEFT);
		} else if(angle_index > 5) {
			g.drawImage(game.a_img_point[angle_index], x_muzzle, y_muzzle, Graphics.TOP | Graphics.LEFT);
		} else {
			g.drawImage(game.a_img_point[angle_index], x_muzzle, y_muzzle, Graphics.VCENTER | Graphics.LEFT);
		}
	}
...
//				game.getCanvasControl().setView(new Dialog(game.getCanvasControl(), 6, null));
				...
				try {
					game.getCanvasControl().zj_tool.do_BuyProp(
		...
		if(showBoom) {
			...
			if (boomFrame > 2) {
				...
				if(life <= 0) {//闯关失败
					...
					Game.shooterIndex = 0;
					game.moveMapRight = false;
					game.moveMapLeft = false;
					game.x_map = 0;
				}
				System.gc();
			} 
