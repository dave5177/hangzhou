...
public class Obstacle {
	 ...
	 * @param y
	 * @param style
	 *            木板的类型 1、是竖的。0、是横的。3、是斜的。
	 * @param layer
	 *            墙壁障碍物的层数,至少两层
	 */
	public Obstacle(Game game, int type, int x, int y, int style, int layer,
			Obstacle nextOst) {
		this.type = type;
		this.game = game;
		this.x = x;
	...
	public void loadImage() {
		if (type == 2) {// 墙壁的图片，要用很多图片拼合而成。
			...
			if (style == WOOD_DOWN || style == WOOD_UP || style == WOOD_T) {
				...
				try {
					for (int i = 0; i < 2; i++) {
						a_img_obstacle[i] = Image
								.createImage("/obstacle/"
										+ CanvasControl.mission + "_wood_" + i
										+ ".png");
					}
				} catch (IOException e) {
					e.printStackTrace();
				...
				try {
					a_img_obstacle[0] = Image
							.createImage("/obstacle/" + CanvasControl.mission
									+ "_wood_" + style + ".png");
	...
	public void show(Graphics g) {
		if (boom) {
			...
			if (type == 2) {// 墙壁的图片，要用很多图片拼合而成。
				...
				if (style == WOOD_DOWN) {
					g.drawImage(a_img_obstacle[1], x - 40 + game.x_map, y,
							Graphics.BOTTOM | Graphics.HCENTER);
					g.drawImage(a_img_obstacle[1], x + 40 + game.x_map, y,
							Graphics.BOTTOM | Graphics.HCENTER);
					g.drawImage(a_img_obstacle[0], x + game.x_map, y - 106,
							Graphics.BOTTOM | Graphics.HCENTER);
				} else if (style == WOOD_UP) {
					g.drawImage(a_img_obstacle[1], x - 40 + game.x_map, y - 15,
							Graphics.BOTTOM | Graphics.HCENTER);
					g.drawImage(a_img_obstacle[1], x + 40 + game.x_map, y - 15,
							Graphics.BOTTOM | Graphics.HCENTER);
					g.drawImage(a_img_obstacle[0], x + game.x_map, y,
							Graphics.BOTTOM | Graphics.HCENTER);
				} else if (style == WOOD_T) {
					g.drawImage(a_img_obstacle[1], x + game.x_map, y,
							Graphics.BOTTOM | Graphics.HCENTER);
					g.drawImage(a_img_obstacle[0], x + 31 + game.x_map,
							y - 217, Graphics.BOTTOM | Graphics.HCENTER);
				} else {
					g.drawImage(a_img_obstacle[0], x + game.x_map, y,
							Graphics.BOTTOM | Graphics.HCENTER);
				}
	...
	public void showWall(Graphics g) {
		...
		for (int i = 0; i < (layer - 3) / 2 + 1; i++) {
			g.drawImage(a_img_obstacle[2], x - 20 + game.x_map,
					y - 19 - i * 18, Graphics.BOTTOM | Graphics.LEFT);
		}
		for (int i = 0; i < (layer - 2) / 2; i++) {
			g.drawImage(a_img_obstacle[1], x - 20 + game.x_map,
					y - 28 - i * 18, Graphics.BOTTOM | Graphics.LEFT);
		}
		g.drawImage(a_img_obstacle[0], x - 20 + game.x_map, y - 19
				- (layer - 2) * 9, Graphics.BOTTOM | Graphics.LEFT);
	}
	...
	private void initTrips() {
		...
		case 1:
			...
			v_fragment.addElement(new Fragment(1, x - 20, y - 40, 2, HSProp
					.getRandom(0, 2) + 1, a_img_chip[HSProp.getRandom(0, 1)],
					game));
			v_fragment.addElement(new Fragment(1, x + 20, y - 40, 8, HSProp
					.getRandom(0, 2) + 1, a_img_chip[HSProp.getRandom(0, 1)],
					game));
			v_fragment.addElement(new Fragment(1, x - 20, y, 4, HSProp
					.getRandom(0, 2) + 1, a_img_chip[HSProp.getRandom(0, 1)],
					game));
			v_fragment.addElement(new Fragment(1, x + 20, y, 6, HSProp
					.getRandom(0, 2) + 1, a_img_chip[HSProp.getRandom(0, 1)],
					game));
			v_fragment.addElement(new Fragment(1, x, y - 20, 7, HSProp
					.getRandom(0, 2) + 1, a_img_chip[HSProp.getRandom(0, 1)],
					game));
			v_fragment.addElement(new Fragment(1, x, y, 5, HSProp.getRandom(0,
					2) + 1, a_img_chip[HSProp.getRandom(0, 1)], game));
			v_fragment.addElement(new Fragment(1, x - 20, y - 20, 3, HSProp
					.getRandom(0, 2) + 1, a_img_chip[HSProp.getRandom(0, 1)],
					game));
			v_fragment.addElement(new Fragment(1, x, y - 40, 1, HSProp
					.getRandom(0, 2) + 1, a_img_chip[HSProp.getRandom(0, 1)],
					game));
			break;
		case 2:
			...
			for (int i = 0; i < layer; i++) {
				v_fragment.addElement(new Fragment(1, x, y - i * 12, HSProp
						.getRandom(0, 7) + 1, HSProp.getRandom(0, 2) + 1,
						a_img_chip[HSProp.getRandom(0, 1) + 1], game));
			}
			break;
		case 3:
			v_fragment = new Vector();
			for (int i = 0; i < 20; i++) {
				v_fragment.addElement(new Fragment(type, x + 25
						- HSProp.getRandom(0, 49), y - HSProp.getRandom(0, 49), HSProp
						.getRandom(0, 7) + 1, HSProp.getRandom(0, 2) + 1,
						a_img_chip[HSProp.getRandom(0, 3)], game));
			}
			break;
		case 4:
			v_fragment = new Vector();
			for (int i = 0; i < 15; i++) {
				v_fragment.addElement(new Fragment(type, x + 20
						- HSProp.getRandom(0, 39), y - HSProp.getRandom(0, 39),
						HSProp.getRandom(0, 7) + 1, HSProp.getRandom(0, 2) + 1,
						a_img_chip[HSProp.getRandom(0, 3)], game));
			}
			break;
		case 5:
			v_fragment = new Vector();
			for (int i = 0; i < 20; i++) {
				v_fragment.addElement(new Fragment(type, x + 4
						- HSProp.getRandom(0, 4), y - height_cln / 2 + 4
						- HSProp.getRandom(0, 4), HSProp.getRandom(0, 7) + 1,
						HSProp.getRandom(0, 2) + 1, a_img_chip[HSProp
								.getRandom(0, 2)], game));
			}
			break;
		...
		try {
			...
			case 2:
				...
				for (int i = 0; i < a_img_chip.length; i++) {
					a_img_chip[i] = Image.createImage("/obstacle/brick_" + i
							+ ".png");
				}
				break;
			case 1:
