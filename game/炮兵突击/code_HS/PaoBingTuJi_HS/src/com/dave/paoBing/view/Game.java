...
//		canvasControl.playerHandler.stopByIndex(0);
	...
	public void show(Graphics g) {
		for (int i = 0; i < a_img_map.length; i++) {
			g.drawImage(a_img_map[i], x_map + i * 240, 0, 0);
		}
...
//		}
	...
	public void loadResource() {
		try {
			...
			a_img_map = new Image[4];
			for (int i = 0; i < a_img_map.length; i++) {
				a_img_map[i] = Image.createImage("/game/back_" + i + ".png");
			}

			a_img_hero = new Image[11][3];
			Image temp = null;
			int w, h;
			int i =  hero.angle_index;
//			for (int i = 0; i < 11; i++) {
				temp = Image.createImage("/hero/hero_" + i + ".png");
				w = temp.getWidth();
				h = temp.getHeight();
				for (int j = 0; j < 3; j++) {
					a_img_hero[i][j] = Image.createImage(temp, w * j / 3,0, w / 3, h, 0);
				}
				temp = null;
//			}
		} catch (IOException e) {
