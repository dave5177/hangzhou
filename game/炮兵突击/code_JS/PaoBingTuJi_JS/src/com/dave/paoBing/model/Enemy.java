...
public class Enemy {
	...
	private void fire() {
		...
		if(C.R.nextInt(100) < acc) {
			...
			if(C.R.nextInt(2) == 1) {
				game.bullet = new Bullet(x - 20, y - 45, game.hero.x, game.hero.y + C.R.nextInt(100) + 20, game, type);
			} else {
				game.bullet = new Bullet(x - 20, y - 45, game.hero.x, game.hero.y - 80 - C.R.nextInt(200), game, type);
			}
		}
		afterFier = true;
	}
