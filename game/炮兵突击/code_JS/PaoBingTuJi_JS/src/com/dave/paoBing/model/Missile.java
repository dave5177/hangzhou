...
public class Missile {
	 ...
	 *            导弹生成的角度值
	 */
	public Missile(Game game, int x, int y, int angle, int power, boolean doublePower) {
		this.game = game;
		this.angle = angle;
		setStartPoint(x, y);
		this.power = power;
	...
	private void initImage() {
		try {
			if(doublePower)
				img_missile = Image.createImage("/missile/" + angle + "_double.png");
			else 
	...
	public void show(Graphics g) {
		...
		if (showBoom && a_img_boom != null) {
			g.drawImage(a_img_boom[boomFrame], x_boom + game.x_map, y_boom, Graphics.VCENTER
					| Graphics.HCENTER);
		}
		
		if (power > 0) {
			if (angle < 0) {
				g.drawImage(img_missile, x + game.x_map, y, Graphics.BOTTOM | Graphics.RIGHT);
			} else if (angle > 0) {
				g.drawImage(img_missile, x + game.x_map, y, Graphics.TOP | Graphics.RIGHT);
			} else {
