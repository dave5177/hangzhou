...
//		init();
	...
	public void showMe(Graphics canvas) {
		...
		drawCurrentScore(canvas);//画用户当前所得分
...
//			s.repaint();
	...
	public void drawCurrentScore(Graphics canvas){
		...
		C.DrawNumber_XY_RIGHTTOP(imagea_score, C.challenge_Score, 540, 32, 17, canvas);//画当前关的游戏分数
	...
	public void drawGameBackGround(Graphics canvas){
		...
		C.DrawImage_XY_LEFTTOP(imagea_gameBack[0], 0,73+backgroundDiZheng, canvas);// 游戏打地鼠区域
		C.DrawImage_XY_LEFTTOP(imagea_gameBack[2], 0,448, canvas);//游戏第三区域，最下面的进度条
		C.DrawNumber_XY_RIGHTTOP(imagea_score, live, 160, 490, 17, canvas);//画生命
		C.DrawNumber_XY_RIGHTTOP(imagea_score, error, 315, 490, 17, canvas);//画错打
		C.DrawNumber_XY_RIGHTTOP(imagea_score, lost, 459, 490, 17, canvas);//画丢失
	}
